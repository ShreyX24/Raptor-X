...
class TimelineManager:
    ...
    def __init__(self, run_id: str, run_dir: str, on_event: Callable[[TimelineEvent], None] = None):
        ...
        # Timeline file path
        self.timeline_file = os.path.join(run_dir, 'blackbox', 'timeline.json')

        # Ensure blackbox directory exists
    ...
    def step_started(self, step_num: int, description: str, total_steps: int = None):
        """Log step started"""
        msg = f"Step {step_num}"
        if total_steps:
            msg += f"/{total_steps}"
